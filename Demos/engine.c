 ...
 *  2. In the DATABASE CONNECTION line 411, modify [dbname], [host], [port], [username], [password] to
 *     yours ([dbname] is the same as your [username] by default).
 *  3. Preprocessor 		- $ ecpg -I /usr/include/postgresql engine.pgc
 *  4. Compile     		- $ g++ -c -I /usr/include/postgresql/ engine.c
 *  5. Link        		- $ g++ -L /usr/include/postgresql/ engine.o -lecpg -o engine
 *  6. Execute			- $ ./engine <EMFquery.txt >program.pgc	(The program.pgc file is the output program file that generated by 
 ...
 *  7. Run program		- read the instructions in the program file
 */
...
void operandProcess() //  assigned Yanjun Wu
{
	...
	while(!ss.eof())
	{
		...
		if(temp=="") continue;
		if(temp[temp.size()-1]==',') temp.erase(temp.size()-1,1);
		string tempSum, tempCount, tempAvg, tempMax, tempMin;
		if(temp.find("sum")!=-1) tempSum=temp;
		else if(temp.find("count")!=-1) tempCount=temp;
		else if(temp.find("max")!=-1)	// in order to compute max, we add count into aggregate functions first
		{
			tempMax = temp;
			tempCount = temp.replace(temp.find("max"),3,"count");
		}
		else if(temp.find("min")!=-1)	// in order to compute min, we add count into aggregate functions first
		{
			tempMin = temp;
			tempCount = temp.replace(temp.find("min"),3,"count");
		}
		else if(temp.find("avg")!=-1)	// in order to compute avg, we add sum and count into aggregate functions first
		{
			tempAvg = temp;
		...
		if(tempMax!="")
		{
			int i;
			for(i = 0; i < F.size(); i++)
			if(F[i]==tempMax) break;
			if(i==F.size()) F.push_back(tempMax);
		}
		if(tempMin!="")
		{
			int i;
			for(i = 0; i < F.size(); i++)
			if(F[i]==tempMin) break;
			if(i==F.size()) F.push_back(tempMin);
		}
		if(tempAvg!="")
		{
			F.push_back(tempAvg);
...
#line 412 "engine.pgc"
   ...
   // READ RECORDS
   //----------------------------------------------------------------------
   /* declare mycursor cursor for select a . table_name , a . column_name , a . data_type , a . character_maximum_length from information_schema . columns as a , information_schema . tables as b where a . table_name = b . table_name and b . table_schema not in ( 'pg_catalog' , 'information_schema' ) */
#line 418 "engine.pgc"
...
#line 419 "engine.pgc"
   ...
   { ECPGdo(__LINE__, 0, 1, NULL, 0, ECPGst_normal, "declare mycursor cursor for select a . table_name , a . column_name , a . data_type , a . character_maximum_length from information_schema . columns as a , information_schema . tables as b where a . table_name = b . table_name and b . table_schema not in ( 'pg_catalog' , 'information_schema' )", ECPGt_EOIT, ECPGt_EORT);
#line 420 "engine.pgc"
...
#line 420 "engine.pgc"
   ...
   { ECPGdo(__LINE__, 0, 1, NULL, 0, ECPGst_normal, "fetch from mycursor", ECPGt_EOIT, 
	ECPGt_char,&(info_schema.table_name),(long)0,(long)1,(1)*sizeof(char), 
	ECPGt_NO_INDICATOR, NULL , 0L, 0L, 0L, 
	ECPGt_char,&(info_schema.column_name),(long)0,(long)1,(1)*sizeof(char), 
	ECPGt_NO_INDICATOR, NULL , 0L, 0L, 0L, 
	ECPGt_char,&(info_schema.data_type),(long)0,(long)1,(1)*sizeof(char), 
	ECPGt_NO_INDICATOR, NULL , 0L, 0L, 0L, 
	ECPGt_short,&(info_schema.character_maximum_length),(long)1,(long)1,sizeof(short), 
	ECPGt_short,&(info_schema.val_len),(long)1,(long)1,sizeof(short), ECPGt_EORT);
#line 423 "engine.pgc"
...
#line 447 "engine.pgc"
 // Close cursor
   ...
   {
	cout << "Table Name: " << endl << tableName << endl <<
	"Columns: " << endl;
	for(int i = 0; i < columnsName.size(); i++)
	cout << columnsName[i] << endl;
	return false;
   }
		...
		if(globalVal) // process for g.v. 0 aggregate functions
		{
			...
			for(int i = 0; i < F.size(); i++) // update the aggregate functions for g.v. 0
			{
				...
				if(F[i].find(stringVar) != -1)
				{
					...
					else if(F[i].find("max") != -1)
					{
						string tempCount = F[i], tempMax = F[i];
						tempCount.replace(tempCount.find("max"), 3, "count");
						tempMax = tempMax.erase(0, tempMax.find_last_of("_") + 1);
						cout << "			if(MFS[i]." + tempCount + " == 1 || MFS[i]."
						+ F[i] + " < sale_rec." + tempMax + ")" << endl <<
						"				MFS[i]." + F[i] + " = sale_rec." + tempMax + ";" << endl;
					}
					else if(F[i].find("min") != -1)
					{
						string tempCount = F[i], tempMin = F[i];
						tempCount.replace(tempCount.find("min"), 3, "count");
						tempMin = tempMin.erase(0, tempMin.find_last_of("_") + 1);
						cout << "			if(MFS[i]." + tempCount + " == 1 || MFS[i]."
						+ F[i] + " > sale_rec." + tempMin + ")" << endl <<
						"				MFS[i]." + F[i] + " = sale_rec." + tempMin + ";" << endl;
					}
			...
			for(int i = 0; i < F.size(); i++) // compute the aggregate functions for the MF variable
			{
				...
				if(F[i].find(stringVar) != -1)
				{
					...
					else if(F[i].find("max") != -1)
					{
						...
						tempCount.replace(tempCount.find("max"), 3, "count");
						tempMax = tempMax.erase(0, tempMax.find_last_of("_") + 1);
						cout << "		if(MFS[hashScan]." + tempCount + " == 1 || MFS[hashScan]."
						+ F[i] + " < sale_rec." + tempMax + ")" << endl <<
						"			MFS[hashScan]." + F[i] + " = sale_rec." + tempMax + ";" << endl;
					}
					else if(F[i].find("min") != -1)
					{
						string tempCount = F[i], tempMin = F[i];
						...
						cout << "		if(MFS[hashScan]." + tempCount + " == 1 || MFS[hashScan]."
						+ F[i] + " > sale_rec." + tempMin + ")" << endl <<
						"			MFS[hashScan]." + F[i] + " = sale_rec." + tempMin + ";" << endl;
					}
					else if(F[i].find("avg") != -1)
					{
						string tempSum = F[i], tempCount = F[i];
...
{	string title = "";
	...
	for(int i = 0; i < S.size() - 1; i++) // output all the needed attributes except the last one
	{
		...
		S[i].find("max") == -1 && S[i].find("min") == -1)
		{
			cout << "%" << S[i].length() << "ld |\", " << "MFS[i]." + S[i] <<  ");" << endl<<1;
		}
		else if(S[i].find("sum") != -1 || S[i].find("avg") != -1 || S[i].find("min") != -1 || S[i].find("max") != -1) 
...
string convertAggregates(string agg, string argu) // assigned Yanjun
{
	...
	if(agg.find_first_of("()+-*/=<>", second) != 0) // check if all the operators are not the first character
	{
		if(agg.find_first_of("()+-*/=<>", second) != -1) // check if any of the operators exist
		{
			...
			else if(tempA.find("max") != -1)
			{
				string tempB = tempA;
				tempB.replace(tempB.find("max"), 3, argu + ".max");
				agg.replace(agg.find(tempA), tempA.length(), tempB);
				second += tempB.length() - tempA.length();
			}
			else if(tempA.find("min") != -1)
			{
				string tempB = tempA;
				tempB.replace(tempB.find("min"), 3, argu + ".min");
				agg.replace(agg.find(tempA), tempA.length(), tempB);
				second += tempB.length() - tempA.length();
			}
		}
